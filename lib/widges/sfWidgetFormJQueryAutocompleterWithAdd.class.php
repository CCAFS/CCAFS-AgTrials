 ...
 */
class sfWidgetFormJQueryAutocompleterWithAdd extends sfWidgetFormInput
{
  /**
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    if($this->getOption('add_new_module') != ''){
        //$display = '<IMG SRC="/images/add-icon.png" ALT="Add New">'.link_to('Add New', $this->getOption('add_new_module').'/new',array('popup'=>array('popupWindow', 'status=no,location=yes,resizable=no,width=800,height=600,left=320,top=0')));
        $display  = "<spam onclick=\"window.open('/".$this->getOption('add_new_module')."/new','popupWindow','status=no,location=yes,resizable=no,width=800,height=600,left=320,top=0')\"> ";
        $display .= "<IMG SRC='/images/add-icon.png' alt='Add New'>";
        $display .= "</spam>";
