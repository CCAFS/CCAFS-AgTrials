...
class sfGuardSecurityUser extends sfBasicSecurityUser {
    ...
    public function initialize(sfEventDispatcher $dispatcher, sfStorage $storage, $options = array()) {
        ...
        //AQUI DESTRUIMOS LA SESSION SI SE CAMBIA DE APLICACION
        if ((!$this->isAuthenticated()) || (sfConfig::get("sf_app") != $this->getAttribute('App'))) {
    ...
    public function signIn($user, $remember = false, $con = null) {
        ...
        $PartFirstName = explode(" ", $user->getFirstName());
        $PartLastName = explode(" ", $user->getLastName());
        $CompleteName = $PartFirstName[0] . " " . $PartLastName[0];
        $this->setAttribute('CompleteName', $CompleteName);
        //REGISTRO DE LA URL EN SESION PARA MULTIPLES ALICACIONES
        $this->setAttribute('App', sfConfig::get("sf_app"));
        ...
        $user->save($con);
        $SfGuardUserInformation = Doctrine::getTable('SfGuardUserInformation')->findOneByUserId($user->getId());

        if (($user->getEmailAddress() == "") || (strpos($user->getEmailAddress(), "none"))) {
            $FlagInfo = false;
            $MsjInfo .= "* Email Address \\n";
        }

        if ($SfGuardUserInformation->getIdInstitution() == "") {
            $FlagInfo = false;
            $MsjInfo .= "* Institution or Affiliation \\n";
        }
        ...
        if ($SfGuardUserInformation->getCity() == "") {
            $FlagInfo = false;
            $MsjInfo .= "* City \\n";
        }
        if ($SfGuardUserInformation->getAddress() == "") {
            $FlagInfo = false;
            $MsjInfo .= "* Address \\n";
        }
        if ($SfGuardUserInformation->getTelephone() == "") {
            $FlagInfo = false;
            $MsjInfo .= "* Telephone \\n";
        }

        if (!$FlagInfo) {
            echo "<script> alert('Your information is incomplete, please check your profile! \\n\\n$MsjInfo'); window.location='/sfGuardUser/user';</script>";
        }

        $SfGuardUserInformation->setVisits($SfGuardUserInformation->getVisits() + 1);
