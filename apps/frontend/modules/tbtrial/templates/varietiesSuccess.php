...
<script src="/js/jquery.tbtrial.js" type="text/javascript"></script>
...
<script>
    $(document).ready(function() {
        ...
        $('#ButtonSaveClosed').click (function() {
            $('#form_varieties').submit();
        });
        $('#ButtonResetAll').click (function() {
            window.parent.ResetListVarieties();
            $("#DivPPal").load('/tbtrial/VarietiesList/');
            $("#DivSelecionadas").html("");
        });
    });

    function SelectVarieties(check){
        ...
        if(check.checked){
            $("#DivPPal").load('/tbtrial/VarietiesList/txt_filtar/'+txt_filtar+'/Valor/'+Valor+'/Accion/Seleccionar');
            $("#DivSelecionadas").load('/tbtrial/SelectVarieties/Valor/'+Valor);
        }

    }

    function RemoveVarieties(check){
        var Valor = check.value;
        var txt_filtar = $('#txt_filtar').attr('value');
        if(txt_filtar == ''){
            txt_filtar = '9999';
        ...
        if(!(check.checked)){
            $("#DivPPal").load('/tbtrial/VarietiesList/txt_filtar/'+txt_filtar+'/Valor/'+Valor+'/Accion/Quitar');
            $("#DivSelecionadas").load('/tbtrial/RemoveVarieties/Valor/'+Valor);
        }
    }
...
<div class="sf_admin_edit ui-widget ui-widget-content ui-corner-all" id="sf_admin_container">
    ...
    <div id="sf_admin_content">
        <div class="sf_admin_form">
            <br>
            <form id="form_varieties" name="form_varieties" action="<?php echo url_for('@varietiessave'); ?>" enctype="multipart/form-data" method="post">
                <table width="100%" cellspacing="1" cellpadding="10" border="1">
                    <tr>
                        <td colspan="4"><b style="font-size: 13px;">Filter: </b><input type="text" id="txt_filtar" name="txt_filtar" size="50" autocomplete="off" placeholder="Enter first letters of the variety name..." style="background-color: #F9EBB2; border: 2px solid #FFD511; font-size: 12px;">&ensp;<span id="Div_Filter" style="display:none;"><?php echo image_tag('loading4.gif', array('size' => '15x15')); ?></span><span id="Div_Filter_OK" style="display:none;"><?php echo image_tag('success.png', array('size' => '15x15')); ?></span></td>
                    </tr>
                    <tr align="center"><td colspan="4" align="center">&ensp;</td></tr>
                    <tr bgcolor="#C7C7C7">
                        <td width="<?php echo $width1; ?>"><label></label></td>
                        <td width="<?php echo $width2; ?>"><label>Name</label></td>
                        <td width="<?php echo $width3; ?>"><label>Origin</label></td>
                        <td width="<?php echo $width4; ?>"><label>Synonym</label></td>
                    </tr>
                    <tr>
                        <td colspan="4">
                            <div id="DivPPal">
...
<?php
                                ...
                                function Varieties() {
                                    ...
                                    $session_varieties_campo = $user->getAttribute('varieties_id');
                                    $NotIn = "";
                                    if (count($session_varieties_campo) > 0) {
                                        foreach ($session_varieties_campo AS $Value) {
                                            $NotIn .= "$Value,";
                                        }
                                    ...
                                    if (strpos($WhereList, "AND TVM.id_variablesmeasured IN")) {
                                        $QUERY = "SELECT V.id_variety,V.vrtname,O.orgname,V.vrtsynonymous ";
                                        $QUERY .= "FROM tb_variety V ";
                                        $QUERY .= "INNER JOIN tb_trialvariety TV ON V.id_variety = TV.id_variety ";
                                        $QUERY .= "INNER JOIN tb_trial T2 ON TV.id_trial = T2.id_trial ";
                                        $QUERY .= "INNER JOIN tb_trialvariablesmeasured TVM ON T2.id_trial = TVM.id_trial ";
                                        $QUERY .= "LEFT JOIN tb_origin O ON V.id_origin = O.id_origin ";
                                        $QUERY .= "WHERE true $WhereList $AndNotIn ";
                                        $QUERY .= "GROUP BY V.id_variety,V.vrtname,O.orgname,V.vrtsynonymous ";
                                        $QUERY .= "ORDER BY V.vrtname,O.orgname ";
                                    } else {
                                        $QUERY = "SELECT V.id_variety,V.vrtname,O.orgname,V.vrtsynonymous ";
                                        $QUERY .= "FROM tb_variety V ";
                                        $QUERY .= "INNER JOIN tb_trialvariety TV ON V.id_variety = TV.id_variety ";
