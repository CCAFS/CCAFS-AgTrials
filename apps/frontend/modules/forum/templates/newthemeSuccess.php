...
<script>
    $(document).ready(function() {
        $('#newthemesubmit').click(function() {
            var thmname = $('#thmname').attr('value');
            var id_category = $('#id_category').attr('value');
            if(id_category == ''){
               jAlert('error', 'Category','Invalid', null)
            }else if(thmname == ''){
                jAlert('error', 'Name','Invalid', null)
            }else{
                $('#div_loading').show();
                $('#form_newtheme').submit();
            }
        });
    }); 
</script>
<div class="sf_admin_edit ui-widget ui-widget-content ui-corner-all" id="sf_admin_container">
    <div class="fg-toolbar ui-widget-header ui-corner-all">
        <h1><?php echo image_tag('conversation-icon.png'); ?><a href="forum">Forum / New Theme</a></h1>
    </div>
    <div id="sf_admin_content">
        <div class="sf_admin_form">
            <form id="form_newtheme" name="form_newtheme" action="<?php echo url_for('@newtheme'); ?>" enctype="multipart/form-data" method="post">
                <table id="tablecss" width="25%">
                    <br>
                    <tr>
                        <td id="bold">Category:</td>
                        <td><?php echo select_category(); ?></td>
                    </tr>
                    <tr>
                        <td id="bold">Name:</td>
                        <td><input type="text" name="thmname" id="thmname" size="36"></td>
                    </tr>
                    <tr align="center">
                        <td align="center" colspan="2">
                            <br>
                            <input type="button" value="Send" id="newthemesubmit">
...
function select_category() {
    $QUERY00 = Doctrine_Query::create()
                    ->select("id_category,ctgname")
                    ->from("TbCategory")
                    ->orderBy("ctgname");
    $Resultado00 = $QUERY00->execute();
    $OPTION = "<OPTION VALUE=''></OPTION>";
    foreach ($Resultado00 AS $fila) {
        $OPTION .= "<OPTION VALUE='{$fila['id_category']}'>{$fila['ctgname']}</OPTION>";
    }
    $HTML = "<SELECT NAME='id_category' id='id_category' SIZE='1'>";
    $HTML .= $OPTION;
    $HTML .= "</SELECT>";
    return $HTML;
}
